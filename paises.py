...
def mostrar_populacao(nome_do_pais):
    resposta = requisicao("{}/{}".format(URL_NAME, nome_do_pais)) #resposta = requisicao(f"{URL_NAME} {nome_do_pais}")
    if resposta:
        lista_de_paises = parsing(resposta)
        if lista_de_paises:
            for pais in lista_de_paises:
                print(f" Os país é {pais['name']}, e sua população: {pais['population']}")
        else:
...
if __name__ == "__main__":
    if len(sys.argv) == 1:
        ...
        if argumento1 == "contagem":
            ...
            print(f" Existem {numero_de_paises} países no mundo.")
